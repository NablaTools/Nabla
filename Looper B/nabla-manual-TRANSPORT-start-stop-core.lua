...
local function StoreItemDataDefer()
	...
	if numItems ~= #tItemsData then
		...
		for i=0, numItems-1 do
			...
			local iPos = reaper.GetMediaItemInfo_Value(cItem,"D_POSITION")
			local iLen = reaper.GetMediaItemInfo_Value(cItem,"D_LENGTH")
			local iEnd = iPos+iLen
			local iMeasure = 0
			local retval, lastMeas, cml, fullbeats, cdenom = reaper.TimeMap2_timeToBeats( proj, iEnd )
...
local function MainDefer()
	...
	for i=1, #tItemsData do
		...
		local retval, actMeas, cml, fullbeats, cdenom = reaper.TimeMap2_timeToBeats( proj, pPos )
		if actMeas == tItemsData[i].e then
			tItemsData[i].e = actMeas+1
			if tItemsData[i].a then
				local iLen = reaper.GetMediaItemInfo_Value(tItemsData[i].a,"D_LENGTH")
				reaper.SetMediaItemInfo_Value(tItemsData[i].a,"D_LENGTH", iLen+reaper.TimeMap2_beatsToTime(proj, 0, 1) )
			end
		end
	end
	------------------------------------------------------------------
...
local function main()
	...
	if pState == 0 then
		if linkTriggers == 'true' then
			...
			local scriptName = "Script: Nabla Looper Manual Trigger Actions.lua"
